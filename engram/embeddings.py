# Generate embeddings via Ollama and do semantic search
# Falls back to fulltext if Ollama isn't running
...
def embed_text(text: str) -> Optional[List[float]]:
    ...
    try:
        ...
        print(f"[engram] Embedding timed out (Ollama slow?)", file=sys.stderr)
        return None
    except requests.exceptions.HTTPError as e:
        if e.response and e.response.status_code == 404:
            print(f"[engram] Model '{EMBED_MODEL}' not found. Install it: ollama pull {EMBED_MODEL}", file=sys.stderr)
        return None
