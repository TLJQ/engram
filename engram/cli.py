...
Commands:
  engram shell               - Launch your shell inside the PTY wrapper (recommended)
  engram ask  "<question>"   - Ask a question about your terminal history
  engram log                 - (internal) Log a single command from the shell hook
  engram install             - Add the shell hook to your RC file automatically
  engram history             - Print recent logged commands
  engram search "<query>"    - Full-text search history
  engram index               - Re-index all commands into embeddings
  engram status              - Show DB stats and config
...
def cmd_ask(args):
    ...
    if not hits:
        print("[engram] No relevant history found. Run some commands first, then try `engram index`.")
        sys.exit(0)
...
def cmd_install(args):
    ...
    if "engram hook" in existing:
        print(f"[engram] Hook already present in {rc_file} — nothing to do.")
    else:
